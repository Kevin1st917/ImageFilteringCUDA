...
__device__ void dev_fft_core(floating Xr[], floating Xi[], floating xr[], floating xi[], bluestein_buffers *buffers, size_t N, size_t fftSize)
{
	...
	if (N == 1) { //Base case
		...
		dev_cooley_tuckey(Xr, Xi, xr, xi, buffers, N, fftSize);   //Use Cooley-Tuckey algo if size is a multiple of 2
	else
		dev_bluestein(Xr, Xi, xr, xi, buffers, N, fftSize);  // use Bluestein's algo othwerwise
#if defined(MAGNITUDE) || defined(PHASE)
	for (i = 0; i < N; i++) {  //calculate magnitude and phase
#if defined(MAGNITUDE)
		inst->abs[i] = (floating)sqrt(inst->Re[i] * inst->Re[i] + inst->Im[i] * inst->Im[i]);
...
__device__ void dev_cooley_tuckey(floating Xr[], floating Xi[], floating xr[], floating xi[], bluestein_buffers *buffers, size_t N, size_t fftSize)
{
	...
	//Divide et impera
	dev_fft_core(xr, xi, Xr, Xi, buffers, N2, fftSize);
	dev_fft_core(&xr[N2], &xi[N2], &Xr[N2], &Xi[N2], buffers, N2, fftSize);
	//swap X and x buffers each time and re-use them so that, no additional buffers are created
...
__device__ void dev_bluestein(floating Xr[], floating Xi[], floating xr[], floating xi[], bluestein_buffers *buffers, size_t N, size_t fftSize)
{
	...
	dev_cooley_tuckey(buffers->fyr, buffers->fyi, buffers->yr, buffers->yi, buffers, fftSize, fftSize);  // obtain fft of zero padded y
	...
	dev_cooley_tuckey(buffers->fvr, buffers->fvi, buffers->vr, buffers->vi, buffers, fftSize, fftSize);  // convert v to freq. domain
	...
	dev_cooley_tuckey(buffers->gi, buffers->gr, buffers->fgi, buffers->fgr, buffers, fftSize, fftSize);  //convert result back to sample domain
