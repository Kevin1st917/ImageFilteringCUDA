...
void test_speed_and_correctness(std::string filename) {
    ...
    for (int k = 0; k < kRuns; ++k) {
        ...
        }
        {
			...
			for(int i = 0;i<sqrt(data1.size());i++){
				for(int j = 0;j<sqrt(data1.size());j++){
					if(j == sqrt(data1.size()) - 1){
						outfile<<data[i*sqrt(data1.size())+j]<<endl;
					}else{
						outfile<<data[i*sqrt(data1.size())+j]<<","; 
					}
